...
```
1. model.startsWith("glm-")        → GLM      (사용자 explicit 선택)
2. model.startsWith("claude-haiku-") → Claude  (내부 운영 호출)
3. session hint (from hook)        → hint.backend
4. model.startsWith("claude-")     → Claude   (default)
5. config.defaultBackend           → 최종 fallback
```
