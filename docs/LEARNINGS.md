...
### 6.1 자동 기동 (SessionStart hook) + 자동 복구 (UserPromptSubmit hook)
...
**공유 모듈:** `plugins/glm/hooks/proxy-lifecycle.js`가 `checkPort`/`waitReady`/`spawnProxy`/`ensureProxyRunning`을 export. `session-start.js`와 `route-hook.js`가 재사용. 상태 반환: `"already-up" | "started" | "missing-path" | "unreachable"`.

**UserPromptSubmit 자동 복구 (§10.6):** route-hook이 매 프롬프트 진입부에서 `ensureProxyRunning()` 호출. 프록시가 살아있으면 ~1-5ms TCP 체크만 하고 통과. 죽어있으면 바로 재기동 + readiness 폴링. 프록시가 kill된 채 열려있는 세션도 다음 프롬프트에서 자동 복구되므로 `/exit`+`/resume` 수작업 불필요.
...
| `GLM_PROXY_READY_TIMEOUT_MS` | SessionStart/UserPromptSubmit hook의 proxy readiness 폴링 제한. 기본 3000 |
...
### 10.6 프록시 다운 시 자동 복구 (해결됨, 2026-04-15)

**증상:** `ANTHROPIC_BASE_URL=http://localhost:4000`이 설정된 상태에서 프록시가 죽으면 (dev 재시작, 로그 inode 고아 문제로 kill, OS reboot 직후 등) 열려있던 모든 Claude Code 세션이 ECONNREFUSED로 동작 불능. SessionStart hook은 새 세션에서만 돌아서 `/exit`+`/resume` 수작업이 필요했음.

**해결:** UserPromptSubmit hook(`route-hook.js`)이 classifier 호출 **전에** `ensureProxyRunning()`을 호출. 프록시가 살아있으면 ~1-5ms TCP 프로브만 하고 통과. 죽어있으면 `plugins/glm/hooks/proxy-lifecycle.js`의 공유 로직으로 재기동 + 최대 3초 readiness 폴링.

**한계 (race):** §3.2의 blocking anomaly가 재현되는 경우, hook의 respawn이 끝나기 전에 main API 요청이 먼저 떠서 그 턴은 여전히 502. 다음 턴부터 정상. 일반 turn(~900ms blocking 관측)에서는 respawn 선행 보장.

**가시화:** `plugins/glm/scripts/statusline.js`가 TCP 300ms 프로브로 프록시 상태 확인, down이면 빨강 `proxy down` 표기 (줄 끝, bold red). 사용자가 race 발생 전 인지 가능.

**debug log:** `GLM_HOOK_DEBUG=1`이면 `/tmp/glm-route-hook.log`에 `proxy-health-start/proxy-health-done state=already-up|started|unreachable|missing-path` 기록.
