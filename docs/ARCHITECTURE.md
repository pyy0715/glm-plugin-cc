...
The plugin is one piece per concern: a **local proxy** for model-aware request rewriting, a **UserPromptSubmit hook** for per-turn intent classification, and a **setup skill** for one-shot configuration.
...
### Phase 3 — hook-based intent classification + session safety

Model-name routing alone (`/model glm-5.1` → GLM) requires manual switching every turn. The hook closes that gap:

- `plugins/glm/hooks/route-hook.js` reads each UserPromptSubmit, asks a cheap GLM-4.7 classifier `CODE` or `OTHER`, and POSTs a hint to `/_hint` keyed by `session_id`.
...
A single hint map keyed by `session_id` (parsed from `body.metadata.user_id`) keeps concurrent Claude Code sessions isolated — a hint set in session A never leaks into session B. The hint TTL is 60s; a longer block map (10min, see [Design decision: reactive session block](#8-reactive-session-block)) handles context-overflow learning.
...
|---|---|---|
| 1 | `claude-haiku-*` prefix | CC's own plumbing (title/summary generation). Pin to Claude so GLM quota isn't wasted on ops traffic. |
| 2 | Session block + GLM-bound target | Session already hit GLM overflow (§8). Preempt to Claude until TTL expires. |
| 3 | `glm-*` prefix | User picked GLM explicitly via `/model`. Always GLM (unless blocked). |
| 4 | Session hint (from hook) | Classifier verdict, keyed by `session_id`, 60s TTL. Overrides Claude's *default* model but not an *explicit* `glm-*`/`claude-haiku-*`. |
| 5 | `claude-*` prefix | Falls here when no hint is set. |
| 6 | `config.defaultBackend` | Final fallback, defaults to `claude`. |

**Why `claude-*` sits *after* the hint:** Claude Code always embeds `claude-sonnet-4-6` / `claude-opus-4-6` in requests. If the prefix outranked the hint, the classifier's verdict would always be ignored. `claude-*` is a default, not an explicit pick — the hint gets to override it.
...
- `src/router.js` holds `blockedSessions: Map<sessionId, expiresAt>`, same GC-on-set pattern as the hint map.
...
- `resolve()` checks the block before the hint — if active and the request is GLM-bound (explicit `glm-*` or `hint.backend === "glm"`), route to Claude.
...
- `route-hook.js` calls `ensureProxyRunning()` before the classifier. Healthy proxy: ~1–5ms TCP probe. Dead proxy: respawn + up to 3s readiness poll, hook still finishes inside its 10s budget.
...
- `/_hint` stores and retrieves session hints.
...
**Adjacent CC proxies and wrappers.** None of these classify prompt *intent*; that's the gap this project fills.
...
│   ├── router.js                   session-keyed hints + block + resolve()
│   ├── proxy.js                    upstream piping + OAuth passthrough
│   ├── server.js                   HTTP server (/v1/messages, /_hint, /_status)
...
│   │   ├── classifier.js           CODE/OTHER classifier (glm-4.7 via proxy)
...
└── scripts/
    └── verify-classifier.js        live-API classifier regression suite
```
...
**Open**
- Classifier accuracy tuning — add Korean + reason-annotated-rule regressions to `scripts/verify-classifier.js`.
- Actually measure end-to-end hook latency (estimated 700–900ms; no felt impact yet).
...
- launchd / systemd service files — SessionStart + UserPromptSubmit auto-recovery cover the same ground without requiring OS-specific setup.
